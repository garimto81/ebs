...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                    RFID 프로젝트용 MCU 비교 (MFRC522 호환)                       │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  🎯 Stage 0 권장: ESP32-WROOM-32 (기존 선택 유지)                               │
│     - 이유: WiFi+BT 내장, 풍부한 예제, 최적 가성비                              │
│                                                                                 │
│  ═══════════════════════════════════════════════════════════════════════════   │
│                                                                                 │
│  💰 Tier 1: 초저가 ($2-5)                                                       │
│  ├── ESP8266 (NodeMCU) ⭐⭐⭐    │ WiFi만, BT 없음, 3.3V                         │
│  └── Arduino Nano ⭐⭐           │ WiFi/BT 없음, 5V (레벨 변환 필요!)            │
│                                                                                 │
│  💵 Tier 2: 저가 ($4-8)                                                         │
│  ├── ESP32-C3 ⭐⭐⭐⭐            │ RISC-V, WiFi+BLE, 저전력                      │
│  ├── Raspberry Pi Pico ⭐⭐⭐   │ WiFi 없음, PIO 강점, MicroPython              │
│  └── Raspberry Pi Pico W ⭐⭐⭐⭐│ WiFi 추가, $6                                 │
│                                                                                 │
│  💳 Tier 3: 중저가 ($5-15)                                                      │
│  ├── ESP32-WROOM-32 ⭐⭐⭐⭐⭐    │ 듀얼코어, WiFi+BT, 최다 예제 ← 권장          │
│  ├── ESP32-S3 ⭐⭐⭐⭐           │ AI 가속, USB OTG, 고성능                      │
│  └── ESP32-C6 ⭐⭐⭐⭐           │ WiFi 6, Thread/Zigbee, Matter 지원            │
│                                                                                 │
│  💎 Tier 4: 중가 ($5-30)                                                        │
│  ├── STM32 Blue Pill ⭐⭐⭐     │ ARM Cortex-M3, 산업용 신뢰성                  │
...
│  Stage 0 (현재) - 하드웨어 검증                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: ESP32-WROOM-32 ($5-10)                                            │   │
│  │ 이유: 최다 예제, WiFi+BT 내장, 3.3V, 충분한 성능                        │   │
│  │ 대안: 없음 (변경 불필요)                                                │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 1 - PokerGFX 복제                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 유지: ESP32-WROOM-32                                                    │   │
│  │ 대안: ESP32-S3 - USB OTG 활용 시                                        │   │
│  │ 대안: ESP32-C3 - 전력 최적화 필요 시                                    │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 2 - WSOP+ 연동                                                           │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: ESP32-S3 (AI 기능, 고성능)                                        │   │
│  │ 대안: ESP32-C6 - Matter/Thread 연동 시                                  │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 3 - 자동화 시스템                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: ESP32-S3 + 커스텀 PCB                                             │   │
│  │ 대안: STM32 계열 - 산업용 인증 필요 시                                  │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
|-----|------------|--------|------|
| **ESP32-WROOM-32** | [$4-6](https://www.aliexpress.com/w/wholesale-esp32-wroom-32.html) | [$8-12](https://www.amazon.com/s?k=esp32+wroom+32) | [디바이스마트](https://www.devicemart.co.kr) |
| **ESP32-C3** | [$3-5](https://www.aliexpress.com/w/wholesale-esp32-c3.html) | [$6-10](https://www.amazon.com/s?k=esp32-c3) | - |
| **ESP32-S3** | [$6-10](https://www.aliexpress.com/w/wholesale-esp32-s3.html) | [$10-15](https://www.amazon.com/s?k=esp32-s3) | - |
| **Raspberry Pi Pico W** | - | [$6](https://www.amazon.com/s?k=raspberry+pi+pico+w) | [엘레파츠](https://www.eleparts.co.kr) |
| **STM32 Blue Pill** | [$3-5](https://www.aliexpress.com/w/wholesale-stm32-blue-pill.html) | [$8-12](https://www.amazon.com/s?k=stm32+blue+pill) | - |
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│               RFID 리더 모듈 가격대별 분류 (13.56MHz, 개발용)                   │
├─────────────────────────────────────────────────────────────────────────────────┤
│  ⚠️ 범위: ESP32 연동 가능 + 커스터마이징 가능 제품만 포함                       │
│                                                                                 │
│  💰 Tier 1: 초저가 ($1-5)        │ 🎯 Stage 0 권장                             │
│  ├── MFRC522 (RC522) ⭐⭐⭐⭐⭐     │ ESP32 SPI 직접 연결                         │
│  └── RDM6300 (125kHz 전용)        │ 주파수 불일치로 비권장                      │
│                                                                                 │
│  💵 Tier 2: 저가 ($5-10)          │ 장거리/다기능 필요 시                       │
│  ├── PN5180 ⭐⭐⭐⭐                 │ ESP32 SPI, 장거리 지원                      │
│  └── PN532 클론                   │ ESP32 SPI/I2C/UART                          │
│                                                                                 │
│  💳 Tier 3: 중저가 ($10-25)       │ 안정성/다중 프로토콜                        │
│  ├── PN532 정품 ⭐⭐⭐             │ Adafruit 라이브러리                         │
│  ├── CLRC663                      │ NXP 풀 프로토콜                             │
│  └── PN7150                       │ NFC Forum 완전 지원                         │
│                                                                                 │
│  💎 Tier 4: 중가 ($25-60)         │ 고성능/SDK 포함                             │
│  ├── ST25R3911B ⭐⭐⭐⭐            │ ESP32 샘플 코드 제공                        │
│  └── ST25R3916                    │ EMVCo 인증, STM32/ESP32 지원               │
│                                                                                 │
│  🏆 Tier 5: 고가 ($60-200)        │ 산업용/SDK 포함                             │
│  └── µFR Zero 시리즈 ⭐⭐⭐        │ 완벽한 SDK, UART/USB 지원                   │
│                                                                                 │
│  📋 참고: PC 전용 USB 리더 (ESP32 연결 불가)                                   │
│  └── ACR122U, ACR1552U, RFIDeas pcProx → 테스트/개발 PC 용도만                 │
│                                                                                 │
│  ❌ 제외: 상용 통합 솔루션 (커스터마이징 불가)                                  │
│  └── PokerGFX, RF Poker, Tableswin → 완제품, 개발 불가                         │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: MFRC522 ($1-5)                                                     │   │
│  │ 이유: 최저가, 풍부한 예제, 충분한 성능                                   │   │
│  │ 목표: "카드 스캔 → 화면 표시" 파이프라인 검증                            │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 1 - PokerGFX 복제                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 유지: MFRC522 (기본)                                                     │   │
│  │ 대안: ST25R3911B ($25-40) - 안정성 필요 시                               │   │
│  │ 대안: PN532 정품 ($15) - NFC 연동 필요 시                                │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 2 - WSOP+ 연동                                                           │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: ST25R3916 또는 µFR Zero                                            │   │
│  │ 이유: 장기 안정성, SDK 지원, 확장성                                      │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Stage 3 - 자동화 시스템                                                        │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ 권장: 커스텀 PCB 설계 (µFR Zero 또는 ST25R3916 기반)                     │   │
│  │ 이유: 프로덕션 품질, 대량 생산 고려, 완전한 커스터마이징                 │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
```
  ...
  Yes       No            Yes       No            Yes       No
   ...
   ▼         ▼             ▼         ▼             ▼         ▼
┌──────┐ ┌──────┐     ┌──────┐ ┌──────┐     ┌──────┐ ┌──────┐
│PN5180│ │MFRC522│     │PN532│ │CLRC663│     │µFR   │ │ST25R │
│ $5-8 │ │ $1-5 │     │$10-15│ │$10-15│     │Zero  │ │3911B │
│      │ │ ⭐⭐⭐⭐⭐│     │      │ │      │     │$60+  │ │$25-40│
└──────┘ └──────┘     └──────┘ └──────┘     └──────┘ └──────┘
...
|------|------------|--------|------|
| **MFRC522** | [$1-2](https://www.aliexpress.com/w/wholesale-mfrc522.html) | [$5-8](https://www.amazon.com/s?k=mfrc522) | [디바이스마트](https://www.devicemart.co.kr) |
| **PN5180** | [$5-8](https://www.aliexpress.us/item/3256806226684027.html) | [$10-15](https://www.amazon.com/dp/B09ZTYWDX2) | - |
| **PN532** | $6-10 | [$15-20](https://www.amazon.com/s?k=pn532) | [엘레파츠](https://www.eleparts.co.kr) |
| **ST25R3911B** | - | [$30-50](https://www.amazon.com/s?k=ST25R3911B) | [Waveshare 한국](https://www.waveshare.com) |
