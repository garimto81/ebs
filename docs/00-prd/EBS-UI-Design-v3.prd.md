...
| 적응형 정보 밀도 | BM-2 Bloomberg | 게임 진행에 따라 정보량 자동 조절 |
| 컨텍스트 기반 동적 UI | BM-1 + BM-2 | 게임 상태에 따라 패널 자동 변경 |
...
| 고정 컨트롤 패널 | 컨텍스트 기반 동적 UI (게임 상태 연동) | BM-1 + BM-2 |
...
```
+------------------------------------------------------------------+
| [EBS Logo]  Table: Main Event #3   [Cmd+K]    [🔴 LIVE] [User] |
+----------+---------------------------------------+---------------+
|          |                                       |               |
| SIDEBAR  |         CENTRAL PREVIEW               | CONTEXT PANEL |
|  220px   |     (Live Overlay Preview)             |    320px      |
|          |       16:9 aspect ratio                |               |
| [Tables] |    Chroma-key blue background          | (Game State   |
|  Table 1 |    Real-time GFX rendering             |  dependent    |
|  Table 2 |                                       |  controls)    |
|  Table 3 |                                       |               |
|          |                                       | [Blinds]      |
| -------- |                                       | [Players]     |
...
| [AT]     |                                       |               |
+----------+---------------------------------------+---------------+
| STATUS: RFID ● Connected | Hand #247 | DELAYED 00:28 | Ctrl+?  |
+------------------------------------------------------------------+
```
...
| 인터랙션 | 오버레이 요소 클릭 → 컨텍스트 패널에 해당 요소 설정 표시 |

### 2.4 컨텍스트 패널 (우측 320px)

게임 상태에 따라 **자동으로 변경**되는 제어 영역. PokerGFX의 고정 탭 구조 대신, Bloomberg Terminal의 "적응형 정보 밀도" 패턴을 적용한다.

| 게임 상태 | 컨텍스트 패널 내용 |
|----------|-------------------|
| IDLE (핸드 없음) | I/O 설정, 테이블 구성, 블라인드/게임 타입 변경 |
| SETUP_HAND | 플레이어 좌석 배치, 스택 확인/수정 |
| PRE_FLOP ~ RIVER | 라이브 팟, 플레이어 액션 로그, 오버레이 On/Off 토글 |
| SHOWDOWN | 위너 하이라이트, 팟 분배, 핸드 히스토리 |
| 프리뷰 요소 클릭 시 | 해당 오버레이 요소의 상세 설정 (위치, 크기, 표시 조건) |
...
| 명령 예시 | 동작 |
|----------|------|
| `blinds 100/200` | 블라인드 변경 |
| `game holdem` | 게임 타입 변경 |
| `show leaderboard` | Leaderboard 오버레이 표시 |
| `hide player 3` | 좌석 3 Player Graphic 숨김 |
| `reset` | 현재 핸드 초기화 |
| `theme dark-blue` | 테마 전환 |

**자연어 퍼지 매칭**: `bl 100 200` → `blinds 100/200`으로 해석. 최근 명령 히스토리 표시.
...
```
+------------------------------------------------------------------+
| [Connected ●] [HOLDEM] [100/200/0]  Hand #247   [HIDE] [ADJUST] |
+------------------------------------------------------------------+
|                                                                  |
|                     9-SEAT OVAL LAYOUT                           |
|                                                                  |
...
|                      POT: 1,250                                  |
+------------------------------------------------------------------+
| [CHECK] [BET ▼] [CALL 200] [RAISE ▼] [FOLD] [ALL-IN]  [UNDO]  |
|                                                                  |
| [1/2 POT] [2/3 POT] [POT] [2x POT]   [ 숫자 키패드 ]          |
+------------------------------------------------------------------+
```
...
|  +-----------+    +-----------+    +-----------+                 |
+------------------------------------------------------------------+
| BLINDS: 100/200 Ante 25 | Hand #247 | FIELD: 8/120              |
+------------------------------------------------------------------+ 1030px
| TICKER: scrolling text >>>                                       | 1080px
+------------------------------------------------------------------+
```
...
|              |                                  |                |
+------------------------------------------------------------------+
| BLINDS: 100/200 Ante 25 | Hand #247                             |
+------------------------------------------------------------------+ 1030px
| TICKER: scrolling text >>>                                       | 1080px
+------------------------------------------------------------------+
```
...
```
+-------+---------------------------+
| PHOTO |  Player Name         [FL] |
|  (A)  |  Stack: 45,200    [BTN]  |  (B~G)
|       |  [Ah] [Kd]    RAISE 500  |
+-------+---------------------------+
         (H) Equity Bar (올인 시)
```
...
|                                          |
|          POT: 12,500                     |
|     Side Pot 1: 3,200  |  Side Pot 2: 800 |
|                                          |
...
|            LEADERBOARD                    |
+------+-------------------+-------+-------+
| Rank | Player            | Stack | Wins  |
+------+-------------------+-------+-------+
|  1   | John Smith        | 125K  |   5   |
|  2   | Maria Garcia      |  98K  |   3   |
|  3   | Tanaka Yuki       |  76K  |   2   |
|  ...                                     |
+------+-------------------+-------+-------+
```
...
```
+------------------------------------------------------------------+
| >>> Player X wins $12,500 with Full House  |  Next break in 15m  |
+------------------------------------------------------------------+
```
...
```
+------------------------------------------------------------------+
| P1:Smith 45K | P2:Garcia 98K | P3:Yuki 76K | P4:Lee 55K | ...   |
+------------------------------------------------------------------+
```
