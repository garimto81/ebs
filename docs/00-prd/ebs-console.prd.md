...
| 산출물 | 경로 | 내용 |
|-------|------|------|
| 기능 체크리스트 | `docs/01_PokerGFX_Analysis/PokerGFX-Feature-Checklist.md` | 149개 기능 (P0/P1/P2) |
...
#### v2.0 Operational Excellence (68개 기능)
...
- 방송 연출 — 전환 애니메이션, 바운스 효과, 플레이어 사진/국기
- 보안 강화 — AES-128, DB 암호화
- 고급 출력 — ATEM 연동, Auto Camera Control, Virtual Camera

#### v3.0 EBS Native (7개 + 신규)

> **목표**: PokerGFX에 없는 EBS 고유 기능, RFID/DB 완전 통합

**추가 범위:**
- RFID 완전 통합 — Register Deck (SV-023), Calibrate (SV-024), AUTO 모드 (PS-013)
- 다중 테이블 — MultiGFX (SV-025)
...
```
  ...
  ├─ Video Output 파이프라인 (SV-006, SV-007, SV-008)
  ...
  └─ GFX 설정 (SV-005, SV-012, SV-013, SV-017, SV-019, SV-020)

  Layer 5: 보안 + 딜레이
  ├─ Trustless Mode (SEC-001, SEC-002, SEC-003, SEC-010)
  ├─ Realtime Mode (SEC-004, SEC-005)
  └─ Secure Delay (SV-006, SV-007 연동)
