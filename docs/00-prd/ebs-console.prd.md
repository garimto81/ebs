...
related_docs:
  ...
  - "docs/00-prd/EBS-UI-Design-v3.prd.md"
...
| 진실 공급원 분산 | PRD-0004 + 위성 문서 간 역할 불명확 | 관할 분리로 해소 (아래 고지 참조) |
...
| Server UI 설계 | `docs/00-prd/EBS-UI-Design-v3.prd.md` | Top-Preview 레이아웃 + 오버레이 설계 (v3.0.0) |
...
```
  PokerGFX 역설계 (ebs_reverse)
       ...
       │                        │
       │                        ▼
       └─ UI 분석    → EBS-UI-Design-v3.prd.md (Top-Preview 레이아웃 + 오버레이 설계)
                                │
                                ▼
                    ebs-console.prd.md ◄── 본 문서 (기능 범위/버전 결정)
                                │
                                ▼
...
> **관할 분리**: 기능 범위와 버전 결정(Keep/Defer/Drop) = 이 문서(ebs-console.prd.md). UI 설계 상세(요소 정의, 레이아웃) = PRD-0004. 두 문서는 상호 보완적이며, 각자의 관할을 넘어서지 않는다.
...
| GFX Console | 25 | 3 | 17 | 2 | 3 |
| Security | 11 | 7 | 3 | 1 | 0 |
| Equity & Stats | 19 | 1 | 15 | 0 | 3 |
| Hand History | 11 | 1 | 8 | 0 | 2 |
| Server 관리 | 30 | 12 | 2 | 2 | 14 |
| **합계** | **149** | **66** | **53** | **8** | **22** |

### 3.3 Drop 목록 (22개 배제 확정)

| ID | 기능 | 카테고리 | 배제 사유 |
|----|------|---------|---------|
| SV-001 | 비디오 소스 관리 | Server 관리 | Sources 기능 전체 제거. OBS에서 처리 |
| SV-002 | Auto Camera Control | Server 관리 | overlay annotation Drop 확정 |
| SV-003 | ATEM Control | Server 관리 | NDI 방식으로 대체 |
| SV-004 | Board Sync / Crossfade | Server 관리 | overlay annotation Drop 확정 |
| SV-005 | Chroma Key | Server 관리 | 크로마키는 OBS에서 처리 |
...
- Server 관리 12개 — 해상도, 딜레이 출력, 레이아웃, 전환 애니메이션, 스폰서 로고, 9x16 세로 출력
...
#### v2.0 Operational Excellence (53개 기능)
...
```
  ...
  └─ GFX 설정 (SV-010, SV-012, SV-013, SV-019, SV-020)
...
| 2.1.0 | 2026-03-05 | UI 설계 문서 참조를 EBS-UI-Design-v3.prd.md v3.0.0(Top-Preview 레이아웃)으로 업데이트. related_docs 추가 |
| 2.2.0 | 2026-03-06 | Sources 기능 전체 제거: SV-001/SV-003/SV-005 Drop. v1.0 68→66, v2.0 54→53, Drop 19→22 |
