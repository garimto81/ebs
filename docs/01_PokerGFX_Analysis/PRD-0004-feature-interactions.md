...
source_docs:
  - ref: "PRD-0004"
    path: "docs/01_PokerGFX_Analysis/PRD-0004-EBS-Server-UI-Design.md"
    desc: "UI 설계서 (184개 요소 정의)"
  - ref: "TechSpec"
...
> 이 문서는 PRD-0004-EBS-Server-UI-Design.md의 **184개 UI 요소**에 대한 상세 인터랙션 명세이다.
>
...
| 문서 | 역할 |
|------|------|
| [PRD-0004-EBS-Server-UI-Design.md](PRD-0004-EBS-Server-UI-Design.md) | UI 레이아웃 + Element Catalog (요소 정의) |
...
| M-08 | Secure Delay *(추후 개발)* | Complex |
| M-09 | Preview Toggle | Medium |
| M-10 | Delay Progress *(추후 개발)* | Complex |
...
| 요소 | 변화 |
|------|------|
| M-08 | Secure Delay 활성 시 Broadcast Canvas로 전환 표시 (추후 개발) |
| M-09 | Preview Toggle Off 시 렌더링 중단 (CPU 절약) |
| M-10 | Delay 활성 시 딜레이된 프레임 표시 (추후 개발) |
| O-01 | 해상도 변경 시 Preview 재초기화 (일시 블랙아웃) |
...
**비활성 조건**: M-09 Preview Toggle Off 시 렌더링 중단. GPU 초기화 실패 시 "GPU Error" 메시지 표시.
...
2. 활성화 시: Broadcast Canvas(`canvas_delayed`) 파이프라인이 시작됨
3. `thread_worker_process_delay`가 `MDelayClass` 내부 버퍼에 프레임 저장 시작
4. O-08에서 설정된 딜레이 시간(기본 30분)만큼 버퍼링 후 `delayed_frames` 큐로 출력
5. M-10 Delay Progress가 표시되어 남은 딜레이 시간을 프로그레스바로 보여줌
6. 비활성화 시: Broadcast 파이프라인 중단, 딜레이 버퍼 플러시

**상태 변화**: Broadcast Canvas 활성/비활성 전환. M-10 프로그레스바 표시/숨김.

**영향 요소**:
| 요소 | 변화 |
|------|------|
| M-02 | Broadcast Canvas 출력이 Preview에 반영 |
| M-10 | 딜레이 잔여 시간 프로그레스바 표시/숨김 |
| O-06+O-07 | Delay 파이프라인 활성/비활성 |
| O-08 | 딜레이 시간 설정값 적용 |
...
sequenceDiagram
    participant OP as Operator
    participant SD as Secure Delay (M-08)
    participant DP as Delay Pipeline
    participant BF as MDelayClass Buffer
    participant BC as Broadcast Canvas
    participant PG as Delay Progress (M-10)

    OP->>SD: 체크박스 ON
    SD->>DP: 딜레이 파이프라인 시작
    DP->>BF: 프레임 버퍼링 시작 (O-08 설정 시간)
    BF->>PG: 잔여 시간 표시 시작
    loop 매 프레임
        DP->>BF: Live 프레임 → 버퍼 저장
        BF-->>BC: N초 후 딜레이 프레임 출력
        PG->>PG: 프로그레스바 갱신
    end
    OP->>SD: 체크박스 OFF
    SD->>DP: 파이프라인 중단
...
**비활성 조건**: Delay 파이프라인(O-06, O-07)이 미설정 시 체크박스 비활성
...
**로직**: Preview 렌더링 On/Off 전환. Off 시 GPU 렌더링을 중단하여 CPU/GPU 리소스를 절약한다. 출력 파이프라인(Live)은 영향 없이 계속 동작.

**영향 요소**: M-02 (Preview Panel 렌더링 중단/재개), M-03/M-04 (CPU/GPU 사용률 감소)
...
**트리거**: M-08 Secure Delay 활성 시 자동 표시

**전제조건**: M-08 Secure Delay가 활성 상태

**로직 플로우**:
1. M-08 활성 시 LinearProgressIndicator가 화면에 표시됨
2. `MDelayClass` 버퍼의 현재 채움 비율을 프로그레스바로 표시
3. 프로그레스바 옆에 텍스트로 남은 시간 표시 (예: "28:30 remaining")
4. 버퍼가 가득 차면(설정 딜레이 시간 도달) 프로그레스바 100% + "Buffer Full" 표시
5. O-08에서 딜레이 시간 변경 시 프로그레스바 범위 재계산
6. 딜레이 버퍼 이상 발생 시 프로그레스바가 Red로 전환

**상태 변화**: 버퍼 상태에 따라 색상 변화 (Blue: 채움 중, Green: 가득 참, Red: 이상)

**영향 요소**:
| 요소 | 변화 |
|------|------|
| M-08 | Secure Delay Off 시 프로그레스바 숨김 |
| O-08 | 딜레이 시간 변경 시 프로그레스바 범위 재조정 |

> **추후 개발** 범위의 인터랙션 다이어그램.

```mermaid
sequenceDiagram
    participant BF as Delay Buffer
    participant PG as Delay Progress (M-10)
    participant O8 as Delay Time (O-08)

    O8->>BF: 딜레이 시간 설정 (예: 30분)
    loop 매 초
        BF->>PG: 현재 버퍼 비율 전달
        alt 정상
            PG->>PG: Blue 프로그레스바 갱신
        else 버퍼 가득 참
            PG->>PG: Green + "Buffer Full"
        else 버퍼 이상
            PG->>PG: Red + 경고
        end
    end
    Note over O8,PG: O-08 변경 시 버퍼 리사이징 → 프로그레스바 범위 재계산
```

**비활성 조건**: M-08 Secure Delay 비활성 시 숨김 처리
...
**비활성 조건**: M-09 Preview Toggle Off 시 비활성
...
| S-03 | Settings (per source) | Medium |
| S-04 | Preview (per source) | Simple |
| S-05 | Board Cam Hide GFX | Medium |
| S-06 | Auto Camera Control | Complex |
| S-07 | Camera Mode | Medium |
| S-08 | Heads Up Split | Simple |
| S-09 | Follow Players | Simple |
| S-10 | Follow Board | Simple |
| S-11 | Background Enable | Medium |
| S-12 | Background Colour | Simple |
| S-13 | Switcher Source | Medium |
| S-14 | ATEM Control | Complex |
| S-15 | Board Sync | Simple |
| S-16 | Crossfade | Simple |
| S-17 | Audio Input Source | Medium |
| S-18 | Audio Sync | Simple |
...
**로직**: NDI, 캡처 카드(DeckLink), 네트워크 카메라를 DataTable로 표시. 각 행에 장치명, 해상도, 상태(연결/끊김), 할당 상태 표시. 행 선택 시 S-03 Settings와 S-04 Preview 활성화.

**영향 요소**: S-03 (선택된 소스의 상세 설정), S-04 (선택된 소스의 미니 프리뷰), O-04 (Live 파이프라인 장치 할당), O-06 (Delay 파이프라인 장치 할당, 추후 개발 시)
...
### S-03: Settings (per source)

**트리거**: S-01에서 소스 선택 후 설정 아이콘 클릭 | **전제조건**: S-01에서 소스 선택됨

**로직**: 선택된 비디오 소스의 해상도, 프레임레이트, 크롭 영역을 설정하는 팝업. 변경 즉시 Preview(M-02)에 반영.

**영향 요소**: M-02 (설정 변경 시 Preview 갱신), O-01 (출력 해상도와 소스 해상도 불일치 시 스케일링)

**비활성 조건**: S-01에서 소스 미선택 시

---

### S-04: Preview (per source)
소스별 미니 프리뷰를 IconButton 클릭으로 팝업 표시. S-01에서 선택된 소스의 실시간 입력 영상을 확인. 참조 전용, 다른 요소에 영향 없음.

---

### S-05: Board Cam Hide GFX
...
| S-09 | 플레이어 추적 활성/비활성 |
| S-10 | 보드 추적 활성/비활성 |
| S-14 | ATEM Control 활성 시 스위처를 통한 카메라 전환 |
...
**트리거**: Dropdown 선택 | **전제조건**: S-00 Fill & Key 모드, S-14 ATEM 연결됨

**로직**: ATEM 스위처에 연결된 입력 소스 중 GFX 출력 대상을 선택. 스위처의 DSK(Downstream Key) 레이어에 Fill & Key 신호를 라우팅하는 소스를 지정.

**영향 요소**: S-14 (ATEM 연결 상태에 따라 소스 목록 갱신), O-05/O-07 (Fill & Key 출력 대상)
...
**트리거**: 체크박스 토글 + IP 주소 입력

**전제조건**: S-00에서 Fill & Key 모드 선택

**로직 플로우**:
1. 체크박스 활성화 후 ATEM 스위처 IP 주소 입력
2. 입력 완료(Enter 또는 Focus Out) 시 TCP 연결 시도
...
| O-08 | Secure Delay Time *(추후 개발)* | Complex |
| O-09 | Dynamic Delay *(추후 개발)* | Medium |
| O-10 | Show Countdown *(추후 개발)* | Medium |
| O-11 | Countdown Video *(추후 개발)* | Simple |
| O-12 | Countdown Background *(추후 개발)* | Simple |
| O-13 | Auto Stream *(추후 개발)* | Simple |
| O-14 | Virtual Camera | Simple |
| O-15 | Recording Mode | Medium |
...
**전제조건**: S-01에 비디오 소스 등록됨, S-00 모드에 따라 O-05 활성 여부 결정
...
**비활성 조건**: S-01 비디오 소스 미등록 시 O-04 비활성. S-00이 Fill & Key 외 모드 시 O-05 비활성.
...
**전제조건**: M-08 Secure Delay 활성, S-01 비디오 소스 등록됨
...
| 요소 | 변화 |
|------|------|
| M-08 | Secure Delay 비활성 시 Delay 파이프라인 중단 |
| M-10 | Delay Progress 표시에 영향 |
| O-08 | 딜레이 시간 설정값 적용 |
| O-20 | DeckLink Channel Map에서 포트 충돌 검증 |
| O-04+O-05 | Live 파이프라인과 동일 포트 할당 불가 (추후 개발 시 적용) |
...
**비활성 조건**: M-08 Secure Delay 비활성 시 전체 비활성. S-00 Fill & Key 외 모드 시 O-07 비활성.

---

### O-08: Secure Delay Time

> **추후 개발**: Delay 이중 출력 구조는 현재 범위에서 제외. 추후 개발 시 구현.

**트리거**: NumberInput 값 변경 (1~30분, 기본 30분)

**전제조건**: M-08 Secure Delay 활성
...
4. M-10 Delay Progress의 프로그레스바 범위가 새 딜레이 시간으로 재계산됨
5. 라이브 중 변경 시 확인 다이얼로그: "딜레이 버퍼 리사이징은 방송 지연에 영향을 줄 수 있습니다"

**상태 변화**: 딜레이 버퍼 크기 변경, M-10 프로그레스바 범위 재계산

**영향 요소**:
| 요소 | 변화 |
|------|------|
| M-10 | 프로그레스바 최대값 재설정 |
| M-08 | Secure Delay 비활성 시 설정 무의미 |
...
**비활성 조건**: M-08 Secure Delay 비활성 시
...
**트리거**: 체크박스 토글 | **전제조건**: M-08 Secure Delay 활성
...
**비활성 조건**: S-00 Fill & Key 외 모드 시 숨김
...
**전제조건**: DeckLink 카드 장착, S-00 Fill & Key 모드
...
**비활성 조건**: DeckLink 카드 미감지 시 전체 비활성 + "No DeckLink detected" 메시지. S-00 Fill & Key 외 모드 시 숨김.
...
| G-07 | Heads Up Layout L/R | Medium | Local |
| G-08 | Heads Up Camera | Medium | Local |
| G-09 | Heads Up Custom Y | Simple | Local |
...
| G-21 | Action Clock | Medium | Global |
...
| G-34 | Unknown Cards Blink | Simple | Global |
...
| Y-11 | Secure Delay Folder | Diagnostics | Simple |
| Y-12 | Export Folder | Diagnostics | Simple |
| Y-13 | Allow AT Access | AT | Complex |
| Y-14 | Predictive Bet | AT | Medium |
| Y-15 | Kiosk Mode | AT | Complex |
| Y-16 | MultiGFX | Advanced | Medium |
| Y-17 | Sync Stream | Advanced | Simple |
| Y-18 | Sync Skin | Advanced | Simple |
| Y-19 | No Cards | Advanced | Medium |
| Y-20 | Disable GPU | Advanced | Medium |
| Y-21 | Ignore Name Tags | Advanced | Simple |
| Y-22 | Auto Start | Advanced | Simple |
| Y-23 | Stream Deck | Advanced | Medium |
