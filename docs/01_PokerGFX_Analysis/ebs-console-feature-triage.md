...
| ID | 기능 | 결정 | 버전 | 근거 |
|----|------|:----:|:----:|------|
| SV-001 | 비디오 소스 관리 | Keep | v1.0 | 카메라/캡처 디바이스 관리 — 방송 입력 필수 |
| SV-002 | Auto Camera Control | Defer | v2.0 | 게임 상태 기반 자동 전환 — 방송 품질 향상 기능 |
| SV-003 | ATEM Control | Defer | v2.0 | 외부 하드웨어 스위처 연동 — 고급 방송 장비 |
| SV-004 | Board Sync / Crossfade | Defer | v2.0 | 밀리초 보드 싱크 — 방송 품질 향상 |
| SV-005 | Chroma Key | Keep | v1.0 | 크로마키 배경 — OBS 오버레이 연동 필수 |
| SV-006 | Live/Delay 이중 출력 | Keep | v1.0 | Live와 Delay 독립 출력 — 방송 필수 |
| SV-007 | Secure Delay 설정 | Keep | v1.0 | 딜레이 시간 설정 — Trustless Mode 운영 필수 |
| SV-008 | Video Size / Frame Rate | Keep | v1.0 | 해상도/프레임레이트 — 방송 출력 기본 설정 |
| SV-009 | Virtual Camera | Defer | v2.0 | OBS 가상 카메라 — 방송 워크플로우 향상 |
| SV-010 | 9x16 Vertical | Drop | — | 세로 모드 출력 — 쇼츠/모바일용, EBS 방송 범위 외 |
...
| SV-025 | MultiGFX | Defer | v3.0 | 다중 테이블 운영 — 인프라 확장 단계 |
| SV-026 | Stream Deck 연동 | Defer | v2.0 | Elgato Stream Deck — 방송 편의 기능 |
...
| SV-010 | 9x16 Vertical | 세로 모드 출력은 쇼츠/모바일 콘텐츠용. EBS는 방송 오버레이가 목적이므로 가로 16:9만 대상 |
