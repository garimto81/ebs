...
> EBS console v1.0의 UI 설계서. PokerGFX 289개 원본 요소를 분석하고, 180개 EBS 요소로 변환하는 과정을 담는다.
>
...
이 문서는 PokerGFX 289개 UI 요소를 분석하고, EBS console v1.0의 180개 요소로 변환하는 설계서다.

### 읽기 순서

| 장 | 내용 |
|:---:|------|
| 1~2 | 변환 개요 — 289→180 수치와 전체 지도 |
| 3~4 | PokerGFX 원본 — 6탭 289개 전수 분석 |
| 5 | 변환 원칙 — 6탭→5탭 매핑 |
...
## 2장. 289개 → 180개: 변환 전체 지도

### 2.1 감축 요인 분해도

```mermaid
flowchart TD
    PGX["PokerGFX 원본<br/>289개 요소"] --> C1["Commentary 탭 배제<br/>-8"]
    PGX --> C2["Main Window Drop<br/>-4 (#4,#8,#9,#10)"]
    PGX --> C3["v17 기능 제거<br/>-10"]
    PGX --> C4["Graphic Editor 통합<br/>-69 (87→18)"]
    PGX --> C5["기타 통합/정리<br/>-5"]
    PGX --> C6["Sources 추가 감축<br/>-6 (S-19~S-24)"]
    PGX --> C7["Outputs 미카운트 보정<br/>-3"]
    PGX --> C8["라이선스/기타 Drop<br/>-6"]
    C1 --> SUM["소계<br/>-111"]
    C2 --> SUM
    C3 --> SUM
    C4 --> SUM
...
| **합계** | **289개** | (행별 합산: 10+30+16+28+20+24+29+8+37+87=289. v29.0에서 Sources 오버레이 1:1 확장 반영: S-03/S-04/S-14 그룹핑 해제 + S-25~S-29 신규 추가) |
...
| S-02 | Video Sources | Add Button | TextButton | NDI 자동 탐색 또는 수동 URL | #22 | 7장 Defer |
| S-03 | Video Sources | L Column | DataColumn | 좌측 비디오 소스 할당 (X 표시). 매뉴얼: "click both the Left and Right columns for the desired source." (p.35) | #5 | 7장 Keep |
| S-04 | Video Sources | Format/Input/URL | DataColumn | 소스 포맷 및 입력 URL 표시 | #3 | 7장 Keep |
...
| S-07 | Camera | Mode | Dropdown | Static / Dynamic. 매뉴얼: "To display video sources in rotation, select 'Cycle' mode instead of 'Static'." (p.35) | #13 | 7장 Defer |
| S-08 | Camera | Heads Up Split | Checkbox | 헤즈업 화면 분할. 매뉴얼: "When play is heads up, and both players are covered by separate cameras, a split screen view showing each player will automatically be displayed." (p.37) | #14 | 7장 Defer |
...
| S-11 | Background | Enable | Checkbox | 크로마키 활성화. 매뉴얼: "To enable chroma key, enable the 'Chroma Key' checkbox." (p.39) | #21 | 7장 Keep |
| S-12 | Background | Background Colour | ColorPicker | 배경색 (기본 Blue). 매뉴얼: "repeatedly click the 'Background Key Colour' button until the desired colour is selected." (p.39) | #20 | 7장 Keep |
| S-13 | External | Switcher Source | Dropdown | ATEM 스위처 연결 (Fill & Key 필수). 매뉴얼: "When using a camera source for video capture from an external vision switcher, select this capture device using the 'External Switcher Source' dropdown box." (p.38) | #26 | 7장 Keep |
| S-14 | External | ATEM Control | Checkbox | ATEM 스위처 제어 활성화 (Fill & Key 필수). 매뉴얼: "PokerGFX can control a Blackmagic ATEM Video Switcher to automatically switch camera inputs to follow the action." (p.40) | #27 | 7장 Keep |
...
| S-25 | Video Sources | R Column | DataColumn | 우측 비디오 소스 할당 (X 표시) | #6 | 7장 Keep |
| S-26 | Video Sources | Cycle Column | DataColumn | 소스 순환 표시 시간 (초 단위, 0=제외). 매뉴얼: "Enter the number of seconds that each video source should be displayed in the 'Cycle' column." (p.35) | #7 | 7장 Keep |
| S-27 | Video Sources | Status Column | DataColumn | 소스 활성/비활성 상태 (ON/OFF) | #8 | 7장 Keep |
| S-28 | Video Sources | Action (Preview/Settings) | DataColumn | Preview 토글 + Settings 버튼. 매뉴얼: "To edit the properties of the video source, click on the 'Settings' keyword." (p.35) | #4 | 7장 Keep |
| S-29 | External | ATEM IP | TextField | ATEM 스위처 IP 주소 입력 필드 (Fill & Key 필수) | #28 | 7장 Keep |
...
| # | 그룹 | 요소 | 설명 | Ref | EBS |
|:-:|------|------|------|:---:|:---:|
| O-01 | Resolution | Video Size | 1080p/4K 출력 해상도. 매뉴얼: "Select the desired resolution and frame rate of the video output." (p.42) | #2 | 7장 Keep |
| O-02 | Resolution | 9x16 Vertical | 세로 모드 (모바일). 매뉴얼: "PokerGFX supports vertical video natively by enabling the '9x16 Vertical' checkbox." (p.43) | #3 | 7장 Keep |
| O-03 | Resolution | Frame Rate | 30/60fps | #4 | 7장 Keep |
| O-04 | Live | Video/Audio/Device | Live 파이프라인 3개 드롭다운. 매뉴얼: "Sends the live and/or delayed video and audio feed to a Blackmagic Decklink device output (if installed), or to an NDI stream on the local network." (p.42) | #5,#7,#9 | 7장 Keep |
| O-05 | Live | Key & Fill | Live Fill & Key 출력 (DeckLink 채널 할당). 매뉴얼: "When an output device that supports external keying is selected, the 'Key & Fill' checkbox is enabled. Activating this feature causes separate key & fill signals to be sent to 2 SDI connectors on the device." (p.43) | #11 | 7장 Keep |
| O-06 | Delay | Video/Audio/Device | Delay 파이프라인 (Live와 독립) | #6,#8,#10 | 7장 Defer |
| O-07 | Delay | Key & Fill | Delay Fill & Key 출력 (DeckLink 채널 할당) | #12 | 7장 Defer |
...
| G-02 | Player Layout | Dropdown | 플레이어 배치. Horizontal, Vert/Bot/Spill, Vert/Bot/Fit, Vert/Top/Spill, Vert/Top/Fit | #4 | 8장 Keep |
| G-03 | X Margin | NumberInput | 좌우 여백 (%, 기본 0.04). 매뉴얼: "This setting controls the size of the horizontal margins. Valid values are between 0 and 1." (p.49) | #41 | 8장 Keep |
| G-04 | Top Margin | NumberInput | 상단 여백 (%, 기본 0.05) | #44 | 8장 Keep |
| G-05 | Bot Margin | NumberInput | 하단 여백 (%, 기본 0.04) | #47 | 8장 Keep |
| G-06 | Leaderboard Position | Dropdown | 리더보드 위치. 매뉴얼: "Selects the position of the Leaderboard graphic." (p.49) | #14 | 8장 Keep |
...
| G-10 | Sponsor Logo 1 | ImageSlot | Leaderboard 스폰서. 매뉴얼: "Displays a sponsor logo at the top of the Leaderboard. NOTE: Pro only." (p.50) | #35 | 8장 Keep |
| G-11 | Sponsor Logo 2 | ImageSlot | Board 스폰서. 매뉴얼: "Displays a sponsor logo to the side of the Board. NOTE: Pro only." (p.50) | #36 | 8장 Keep |
| G-12 | Sponsor Logo 3 | ImageSlot | Strip 스폰서. 매뉴얼: "Displays a sponsor logo at the left-hand end of the Strip. NOTE: Pro only." (p.50) | #37 | 8장 Keep |
| G-13 | Vanity Text | TextField+Checkbox | 테이블 텍스트 + Game Variant 대체 | #39~#40 | 8장 Keep |

##### Visual 그룹 (연출)

| # | 요소 | 타입 | 설명 | Ref | EBS |
|:-:|------|------|------|:---:|:---:|
| G-14 | Reveal Players | Dropdown | 카드 공개 시점. 매뉴얼: "Determines when players are shown: Immediate / On Action / After Bet / On Action + Next" (p.50) | #6 | 8장 Keep |
| G-15 | How to Show Fold | Dropdown+NumberInput | 폴드 표시. Immediate / Delayed | #8,#10 | 8장 Keep |
| G-16 | Reveal Cards | Dropdown | 카드 공개 연출. Immediate / After Action / End of Hand / Showdown Cash / Showdown Tourney / Never | #12 | 8장 Keep |
| G-17 | Transition In | Dropdown+NumberInput | 등장 애니메이션 + 시간 | #16 | 8장 Keep |
| G-18 | Transition Out | Dropdown+NumberInput | 퇴장 애니메이션 + 시간 | #20 | 8장 Keep |
| G-19 | Indent Action Player | Checkbox | 액션 플레이어 들여쓰기 | #46 | 8장 Keep |
| G-20 | Bounce Action Player | Checkbox | 액션 플레이어 바운스 | #52 | 8장 Keep |
| ~~G-21~~ | ~~Show Action Camera~~ | ~~Checkbox+NumberInput~~ | ~~액션 카메라 표시 + 타임아웃(초). overlay #60=값 필드, #61=초 단위~~ | ~~#59,#60~~ | ~~Drop~~ |
| G-22 | Show Leaderboard | Checkbox+Settings | 핸드 후 리더보드 자동 표시 | #53,#54 | 8장 Keep |
| G-23 | Show PIP Capture | Checkbox+Settings | 핸드 후 PIP 표시 | #55,#56 | 8장 Defer |
| G-22s | Show Player Stats | Checkbox+Settings | 핸드 후 티커 통계 표시. 통계 시스템 전제. (구 G-24) | #57,#58 | 8장 Defer |
| G-25 | Heads Up History | Checkbox | 헤즈업 히스토리 | #50 | 8장 Defer |

##### Skin 그룹

| # | 요소 | 타입 | 설명 | Ref | EBS |
|:-:|------|------|------|:---:|:---:|
| G-13s | Skin Info | Label | 현재 스킨명 + 용량 (`Titanium, 1.41 GB`) | #32 | 8장 Keep |
| G-14s | Skin Editor | TextButton | 별도 창 스킨 편집기 실행 | #33 | 8장 Defer |
| G-15s | Media Folder | TextButton | 스킨 미디어 폴더 탐색기 | #34 | 8장 Defer |
...
| G-44b | Show Eliminated in Strip | Checkbox | Strip에 탈락 선수 표시 | #7 | 10장 Defer |
...
| Y-02 | Table | Password | 접속 비밀번호. 매뉴얼: "Password for this table. Anyone attempting to use Action Tracker with this table will be required to enter this password." (p.60) | #6 | 11장 Keep |
| Y-03 | RFID | Reset | RFID 시스템 초기화. 매뉴얼: "Resets the RFID Reader connection, as if PokerGFX had been closed and restarted." (p.60) | #3 | 11장 Keep |
| Y-04 | RFID | Calibrate | 안테나별 캘리브레이션. 매뉴얼: "Perform the once-off table calibration procedure, which 'teaches' the table about its physical configuration." (p.60). overlay에서 #3(Reset)과 통합 영역 — #4 별도 라벨 없음 | #3 | 11장 Keep |
| Y-04b | License | Serial Number | 시리얼 번호 표시 (읽기 전용) | #11 | Drop (라이선스) |
| Y-05 | RFID | UPCARD Antennas | UPCARD 안테나로 홀카드 읽기. 매뉴얼: "Enables all antennas configured for reading UPCARDS in STUD games to also detect hole cards when playing any flop or draw game." (p.59) | #16 | 11장 Keep |
| Y-06 | RFID | Disable Muck | AT 모드 시 muck 안테나 비활성. 매뉴얼: "Causes the muck antenna to be disabled when in Action Tracker mode." (p.59) | #17 | 11장 Keep |
| Y-07 | RFID | Disable Community | 커뮤니티 카드 안테나 비활성 | #18 | 11장 Keep |
| Y-04c | License | PRO License | PRO 라이선스 활성화 상태 표시 | #13 | Drop (라이선스) |
| Y-08 | System Info | Hardware Panel | CPU/GPU/OS/Encoder 자동 감지 | #24 | 11장 Defer |
| Y-09 | Diagnostics | Table Diagnostics | 안테나별 상태, 신호 강도 (별도 창). 매뉴얼: "Displays a diagnostic window that displays the physical table configuration along with how many cards are currently detected on each antenna." (p.60) | #23 | 11장 Keep |
| Y-10 | Diagnostics | System Log | 로그 뷰어 | #25 | 11장 Keep |
| ~~Y-11~~ | ~~Diagnostics~~ | ~~Secure Delay Folder~~ | ~~보안 딜레이 녹화 파일 저장 경로~~ | ~~#26~~ | ~~Drop~~ |
| Y-12 | Diagnostics | Export Folder | 내보내기 폴더. 매뉴얼: "When the Developer API is enabled, use this to specify the location for writing the JSON hand history files." (p.60) | #27 | 11장 Keep |
| Y-13 | AT | Allow AT Access | AT 접근 허용. 매뉴얼: "'Track the action' can only be started from Action Tracker if this option is enabled." (p.58) | #20 | 11장 Keep |
| Y-14 | AT | Predictive Bet | 베팅 예측 입력. 매뉴얼: "Action Tracker will auto-complete bets and raises based on the initial digits entered, min raise amount and stack size." (p.60) | #21 | 11장 Keep |
...
| Y-22 | Advanced | Auto Start | OS 시작 시 자동 실행 | #19 | 11장 Keep |
| ~~Y-23~~ | ~~Advanced~~ | ~~Stream Deck~~ | ~~Elgato Stream Deck 매핑~~ | ~~#28~~ | ~~Drop~~ |
| Y-24 | Updates | Version + Check | 버전 표시 + 업데이트 확인 | #12 | 11장 Defer |
...
|-----------------|------|--------|---------|
| Sources | S-00~S-01, S-03~S-04, S-07~S-08, S-11~S-14, S-25~S-29 | I/O | Input 섹션 |
...
| S-03 | L Column | DataColumn | Keep | 좌측 비디오 소스 할당 (X 표시). 매뉴얼: "click both the Left and Right columns for the desired source." (p.35) | S-03 |
...
| S-14 | ATEM Control | Checkbox | Keep (수동만 v1.0) | ATEM 스위처 제어 활성화. Fill & Key 모드에서만 표시. 매뉴얼: "PokerGFX can control a Blackmagic ATEM Video Switcher to automatically switch camera inputs to follow the action." (p.40) | S-14 |
| S-25 | R Column | DataColumn | Keep | 우측 비디오 소스 할당 (X 표시) | S-25 |
| S-26 | Cycle Column | DataColumn | Keep | 소스 순환 표시 시간 (초 단위, 0=제외). 매뉴얼: "Enter the number of seconds that each video source should be displayed in the 'Cycle' column." (p.35) | S-26 |
| S-27 | Status Column | DataColumn | Keep | 소스 활성/비활성 상태 (ON/OFF) | S-27 |
| S-28 | Action (Preview/Settings) | DataColumn | Keep | Preview 토글 + Settings 버튼. 매뉴얼: "To edit the properties of the video source, click on the 'Settings' keyword." (p.35) | S-28 |
| S-29 | ATEM IP | TextField | Keep (수동만 v1.0) | ATEM 스위처 IP 주소 입력 필드. Fill & Key 모드에서만 표시. v1.0은 수동 IP 입력만 구현 | S-29 |
...
> **Drop 확정**: O-08~O-12(Delay 관련), O-14(Virtual Camera), O-15(Recording Mode), O-16~O-17(Streaming).
...
4서브그룹: Layout(G-01~G-06) > Card & Player(G-14~G-16, G-22~G-23) > Animation(G-17~G-20) > Branding(G-10~G-13, G-13s~G-15s, G-22s).
...
| G-02 | Player Layout | Dropdown | Keep | 플레이어 배치 모드 (Horizontal/Vert-Bot-Spill/Vert-Bot-Fit/Vert-Top-Spill/Vert-Top-Fit). 매뉴얼 p.48 | GFX1 #4 |
| G-03 | X Margin | NumberInput | Keep | 좌우 여백 (%, 기본 0.04). 정규화 좌표(0.0~1.0). 매뉴얼: "This setting controls the size of the horizontal margins." (p.49) | GFX1 #41 |
| G-04 | Top Margin | NumberInput | Keep | 상단 여백 (%, 기본 0.05). 매뉴얼 p.49 | GFX1 #44 |
| G-05 | Bot Margin | NumberInput | Keep | 하단 여백 (%, 기본 0.04). 매뉴얼 p.49 | GFX1 #47 |
| G-06 | Leaderboard Position | Dropdown | Keep | 리더보드 위치 (Centre/Left/Right). 매뉴얼: "Selects the position of the Leaderboard graphic." (p.49) | GFX1 #14 |

#### Card & Player 서브그룹 (6개)

| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| G-14 | Reveal Players | Dropdown | Keep | 카드 공개 시점 (Immediate/On Action/After Bet/On Action + Next). 매뉴얼: "Determines when players are shown." (p.50) | GFX1 #6 |
| G-15 | How to Show Fold | Dropdown+NumberInput | Keep | 폴드 표시 방식 (Immediate/Delayed + 초). 매뉴얼: "Player is removed immediately." / "Player graphic displays 'Fold', then disappears after a few seconds." (p.51) | GFX1 #8,#10 |
| G-16 | Reveal Cards | Dropdown | Keep | 카드 공개 연출 (Immediate/After Action/End of Hand/Showdown Cash/Showdown Tourney/Never). 매뉴얼 p.51 | GFX1 #12 |
| ~~G-21~~ | ~~Show Action Camera~~ | ~~Checkbox+NumberInput~~ | ~~Drop~~ | ~~액션 카메라 표시 + 타임아웃(초). overlay #60=값 필드, #61=초 단위. triage: SV-017~~ | ~~GFX1 #59,#60~~ |
| G-22 | Show Leaderboard | Checkbox+Settings | Keep | 핸드 후 리더보드 자동 표시 + 설정. 레이아웃 트리거이므로 Keep 유지 | GFX1 #53,#54 |
| G-23 | Show PIP Capture | Checkbox+Settings | Defer (v2.0) | 핸드 후 PIP 표시. triage: GC-023 Defer — 방송 품질 향상 기능 | GFX1 #55,#56 |

#### Animation 서브그룹 (4개, v1.0 Keep)

| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| G-17 | Transition In | Dropdown+NumberInput | Keep | 등장 애니메이션 + 시간(초). triage: SV-014 Keep (v22.0.0 복원). 매뉴얼 p.48 | GFX1 #16 |
| G-18 | Transition Out | Dropdown+NumberInput | Keep | 퇴장 애니메이션 + 시간(초). triage: SV-014 Keep (v22.0.0 복원) | GFX1 #20 |
| G-19 | Indent Action Player | Checkbox | Keep | 액션 플레이어 들여쓰기 | GFX1 #46 |
| G-20 | Bounce Action Player | Checkbox | Keep | 액션 플레이어 바운스 효과. triage: SV-015 Keep (v22.0.0 복원) | GFX1 #52 |

#### Branding 서브그룹 (8개)

| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| G-10 | Sponsor Logo 1 | ImageSlot | Keep | Leaderboard 위치 스폰서 로고. triage: SV-016 Keep (v22.0.0 복원). 매뉴얼: "Displays a sponsor logo at the top of the Leaderboard. NOTE: Pro only." (p.50) | GFX1 #35 |
| G-11 | Sponsor Logo 2 | ImageSlot | Keep | Board 위치 스폰서 로고. 매뉴얼: "Displays a sponsor logo to the side of the Board. NOTE: Pro only." (p.50) | GFX1 #36 |
| G-12 | Sponsor Logo 3 | ImageSlot | Keep | Strip 위치 스폰서 로고. 매뉴얼: "Displays a sponsor logo at the left-hand end of the Strip. NOTE: Pro only." (p.50) | GFX1 #37 |
| G-13 | Vanity Text | TextField+Checkbox | Keep | 테이블 표시 텍스트 + Game Variant 대체 옵션. 매뉴얼: "Custom text displayed on the Board Card / Pot graphic." (p.49) | GFX1 #39~#40 |
| G-13s | Skin Info | Label | Keep | 현재 스킨명 + 용량 (예: Titanium, 1.41 GB) | GFX1 #32 |
| G-14s | Skin Editor | TextButton | Defer (v2.0) | 별도 창 스킨 편집기 실행. triage: SV-027 Defer | GFX1 #33 |
| G-15s | Media Folder | TextButton | Defer (v2.0) | 스킨 미디어 폴더 탐색기. triage: SV-028 Defer | GFX1 #34 |
| G-22s | Show Player Stats | Checkbox+Settings | Defer (v2.0) | 핸드 후 티커 통계 표시. 통계 시스템 전제. triage: GC-017 Defer | GFX1 #57,#58 |

> **v2.0 Defer**: G-23(Show PIP Capture), G-25(Heads Up History).
> **Drop**: G-07~G-09(Heads Up Layout/Camera/Custom Y), G-21(Show Action Camera).
...
> **v2.0 Defer**: G-26~G-31(Leaderboard 옵션 6개 — Show Knockout Rank/Chipcount %/Eliminated/Cumulative Winnings/Hide/Max BB Multiple, 리더보드 데이터 시스템 전제), G-37(Show Hand Equities — Equity 엔진 전제), G-40~G-42(Outs 3개 — Equity 엔진 연관, 정밀 계산 알고리즘 전제), G-43~G-44b(Score Strip/Order Strip By/Show Eliminated in Strip — Strip 기능 v2.0 Defer), G-57(Ignore Split Pots — Equity/Outs Split pot 처리, Equity 엔진 전제).
...
5구역: Table(Y-01~Y-02, 최상단) + RFID(Y-03~Y-07, 상단) + AT(Y-13~Y-14, 중단) + Diagnostics(Y-09~Y-10, Y-12, 하단) + Startup(Y-22).
...
| Y-02 | Table Password | TextField | Keep | 접속 비밀번호. 매뉴얼: "Password for this table. Anyone attempting to use Action Tracker with this table will be required to enter this password." (p.60) | #6 |
...
| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| Y-03 | RFID Reset | TextButton | Keep | RFID 시스템 초기화 (재시작 없이 연결 재설정). 매뉴얼: "Resets the RFID Reader connection, as if PokerGFX had been closed and restarted." (p.60) | #3 |
| Y-04 | RFID Calibrate | TextButton | Keep | 안테나별 캘리브레이션. 매뉴얼: "Perform the once-off table calibration procedure, which 'teaches' the table about its physical configuration." (p.60). overlay #3(Reset)과 통합 영역 | #3 |
| Y-05 | UPCARD Antennas | Checkbox | Keep | UPCARD 안테나로 홀카드 읽기 활성화. 매뉴얼: "Enables all antennas configured for reading UPCARDS in STUD games to also detect hole cards when playing any flop or draw game." (p.59) | #16 |
| Y-06 | Disable Muck | Checkbox | Keep | AT 모드 시 muck 안테나 비활성. 매뉴얼: "Causes the muck antenna to be disabled when in Action Tracker mode." (p.59) | #17 |
...
| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| Y-13 | Allow AT Access | Checkbox | Keep | AT 접근 허용. 매뉴얼: "'Track the action' can only be started from Action Tracker if this option is enabled. When disabled, Action Tracker may still be used but only in Auto mode." (p.58) | #20 |
| Y-14 | Predictive Bet | Checkbox | Keep | 베팅 예측 자동완성. 매뉴얼: "Action Tracker will auto-complete bets and raises based on the initial digits entered, min raise amount and stack size." (p.60) | #21 |
| ~~Y-15~~ | ~~Kiosk Mode~~ | ~~Checkbox~~ | ~~Drop~~ | ~~AT 키오스크 모드 (서버 시작 시 AT 자동 실행 + 닫기/최소화 불가). 매뉴얼: "When the Server starts, Action Tracker is automatically started on the same PC on the secondary display in kiosk mode." (p.58)~~ | ~~#22~~ |

#### Diagnostics 서브그룹 (3개, v1.0 Keep)

| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| Y-09 | Table Diagnostics | TextButton | Keep | 안테나별 상태/신호 강도 별도 창. 매뉴얼: "Displays a diagnostic window that displays the physical table configuration along with how many cards are currently detected on each antenna." (p.60) | #23 |
| Y-10 | System Log | TextButton | Keep | 로그 뷰어 (실시간 이벤트/오류 로그) | #25 |
| Y-12 | Export Folder | FolderPicker | Keep | JSON 핸드 히스토리 내보내기 폴더 지정. 매뉴얼: "When the Developer API is enabled, use this to specify the location for writing the JSON hand history files." (p.60) | #27 |

#### Startup 서브그룹 (1개, v1.0 Keep)

| # | 요소 | 타입 | v1.0 | 설명 | 원본 |
|:-:|------|------|:----:|------|:----:|
| Y-22 | Auto Start | Checkbox | Keep | OS 시작 시 EBS Server 자동 실행. 매뉴얼: "Automatically start the PokerGFX Server when Windows starts. Useful for unattended installations." (p.58) | #19 |
...
> **Drop (라이선스 4개)**: PokerGFX 라이선스 키/활성화 코드/라이선스 서버/시리얼 번호 관련 항목 — EBS 자체 시스템에서 불필요. 원본 #11~#13 제거 확정.
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| **v29.0.0** | **2026-03-03** | Sources 탭 Element Catalog 오버레이 1:1 확장 (25→30): 3장/4.2절/7장 S-03/S-04/S-14 그룹핑 해제, S-25~S-29 신규 추가. 전역 수치 276/175→289/180 통일, 5.4절 감축 흐름도+테이블 pokergfx-analysis.md 4.4절과 정합성 확보 |
| v28.0.0 | 2026-03-02 | Phase A 정합성 전면 수정: 3장 합계 갱신(Outputs 13→16), S-23/S-24 Drop 확정, Y-12/G-22s ID 충돌 해소, AT-003/004 Drop 매핑 수정, O-16~O-17 Drop/Defer 통일, 5.3절 매핑 테이블 정리, 8장 G-21 레이아웃 수정, 11장 5구역 수정 |
| v27.4.0 | 2026-03-02 | 4장 Element Catalog 재검증 — Ref 4건 수정(S-19 #10 결번 제거, G-21 이름+Ref, G-34 이름, Y-04 #3 통합), 누락 2건 추가(G-44b Show Eliminated in Strip, Y-25 WiFi Drop), 이름 1건 수정(G-34), 8~11장 원본 참조 동기화, 카운트 276→175 |
| v27.3.0 | 2026-03-01 | Element Catalog Ref 컬럼 → overlay 번호 전면 통일: Sources/Outputs/GFX1/System 4개 탭 Ref 전면 교체, 누락 요소 6개 추가(S-19~S-24), 8장/11장 원본 참조 동기화, 274→174 수식 조정 |
