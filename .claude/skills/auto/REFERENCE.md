# /auto REFERENCE - Phase 전환 상세 워크플로우
...
Phase 1 PLAN → Phase 2 DESIGN → Phase 3 DO → Phase 4 CHECK → Phase 5 ACT
  ...
  계획문서        설계문서        구현(Ralph)    QA+이중검증    개선/완료
                                                  |
                                         UltraQA + Architect + gap-detector
```
...
### Step 1.0: 사전 분석 (병렬 Explore)

```
Task(subagent_type="oh-my-claudecode:explore", model="haiku",
     description="[PDCA Plan] 문서 분석",
     prompt="docs/, .claude/ 내 관련 문서 탐색. 중복 범위 감지 필수",
     run_in_background=True)

Task(subagent_type="oh-my-claudecode:explore", model="haiku",
     description="[PDCA Plan] 이슈 분석",
     prompt="gh issue list 실행하여 유사 이슈 탐색. 연관 이슈 태깅 필요",
     run_in_background=True)
```
...
**LIGHT (0-1점): Planner haiku**
```
Task(
  subagent_type="oh-my-claudecode:planner",
  model="haiku",
  description="[PDCA Plan] 간단 기능 계획",
  prompt="... (복잡도: LIGHT {score}/5, 단일 파일 수정 예상)"
)
```

**STANDARD (2-3점): Planner sonnet**
```
Task(
  subagent_type="oh-my-claudecode:planner",
  model="sonnet",
  description="[PDCA Plan] 기능 계획",
  prompt="... (복잡도: STANDARD {score}/5, 판단 근거 포함)"
)
```
...
| Phase | 실행 |
|-------|------|
| Phase 1 | Explore(haiku) x2 + Planner(haiku) |
| Phase 2 | **스킵** (설계 문서 생성 없음) |
| Phase 3 | Task(executor, sonnet) 단일 실행 (Ralph 없음) |
| Phase 4 | UltraQA 1사이클 (E2E 스킵) |
| Phase 5 | gap-detector 스킵 (Architect만 검증) |

### STANDARD 모드 (2-3점)

| Phase | 실행 |
|-------|------|
| Phase 1 | Explore(haiku) x2 + Planner(sonnet) |
| Phase 2 | Architect(sonnet) |
| Phase 3 | Ralph 루프 (5조건 검증) |
| Phase 4 | UltraQA 3사이클 + E2E (있으면) + Architect + gap-detector |
| Phase 5 | gap < 90% → pdca-iterator (최대 5회) |

### HEAVY 모드 (4-5점)

| Phase | 실행 |
|-------|------|
| Phase 1 | Explore(haiku) x2 + Ralplan (Planner+Critic) |
| Phase 2 | Architect(opus) |
| Phase 3 | Ralph 루프 + Ultrawork (병렬 에이전트) |
| Phase 4 | UltraQA 5사이클 + E2E (필수) + Architect(opus) + gap-detector(opus) |
| Phase 5 | gap < 90% → pdca-iterator (최대 5회) |
...
**LIGHT 모드: Task(executor, sonnet) 단일 실행**
```
Task(
  subagent_type="oh-my-claudecode:executor",
  model="sonnet",
  description="[PDCA Do] 단일 파일 구현",
...
**Ralph 루프 내부 동작:**
```
1. Ultrawork 활성화 (병렬 에이전트 실행)
   - 독립 작업: run_in_background: true
   - 의존 작업: 순차 실행
...
### Step 4.2: 이중 검증 (병렬 실행)

**LIGHT 모드: Architect만 (gap-detector 스킵)**
```
Task(subagent_type="oh-my-claudecode:architect", model="sonnet",
     description="[PDCA Check] 기능 완성도 검증",
     prompt="구현된 기능이 docs/01-plan/{feature}.plan.md 요구사항과 일치하는지 검증")
```

**STANDARD/HEAVY 모드: Architect + gap-detector**
```
Task(subagent_type="oh-my-claudecode:architect",
     model="sonnet" if STANDARD else "opus",
     description="[PDCA Check] 기능 완성도 검증",
     prompt="구현된 기능이 docs/02-design/{feature}.design.md와 일치하는지 검증")

Task(subagent_type="bkit:gap-detector",
     model="sonnet" if STANDARD else "opus",
     description="[PDCA Check] 설계-구현 갭 분석",
...
| Check 결과 | 자동 실행 | 다음 |
|-----------|----------|------|
| gap < 90% | bkit:pdca-iterator (최대 5회 반복) | Phase 4 재실행 |
| gap >= 90% + Architect APPROVE | bkit:report-generator | 완료 |
| Architect REJECT | oh-my-claudecode:executor (수정) | Phase 4 재실행 |

**Case 1: gap < 90%**
```
Task(
  subagent_type="bkit:pdca-iterator", model="sonnet",
...
```
Task(
  subagent_type="bkit:report-generator", model="haiku",
...
**신규 기능:**
- 복잡도 자동 판단 (LIGHT/STANDARD/HEAVY)
- 모드별 에이전트 모델 최적화
- 자동 승격 규칙 (빌드 실패, UltraQA 초과, 파일 영향도)
