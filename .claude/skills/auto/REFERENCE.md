...
| Phase 4 | UltraQA 3사이클 + E2E (있으면) + Architect teammate + gap-detector teammate |
...
| Phase 4 | UltraQA 5사이클 + E2E (필수) + Architect teammate (opus) + gap-detector teammate (opus) |
| Phase 5 | gap < 90% → pdca-iterator teammate (최대 5회) |
...
**STANDARD 모드: Architect → gap-detector (순차 teammate)**
```
...
```
# 1. Architect teammate (opus) 먼저 → 2. gap-detector teammate (opus) 다음
```
